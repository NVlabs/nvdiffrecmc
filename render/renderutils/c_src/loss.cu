...
__device__ inline float fwdRELMSE(float img, float target, float eps = 0.01f)
{
    return (img - target) * (img - target) / (img * img + target * target + eps);
}

__device__ inline void bwdRELMSE(float img, float target, float &d_img, float &d_target, float d_out, float eps = 0.01f)
{
...
__device__ inline float fwdN2N(float img, float target, float eps = 0.01f)
{
    return (img - target) * (img - target) / (img * img + eps);
}

__device__ inline void bwdN2N(float img, float target, float &d_img, float &d_target, float d_out, float eps = 0.01f)
{
    float denom  = (img * img + eps);
    d_img    += d_out * 2 * (img - target)  / denom;
    d_target -= d_out * 2 * (img - target)  / denom;
}
...
__global__ void imgLossFwdKernel(LossKernelParams p)
{
    ...
    if (px < p.gridSize.x && py < p.gridSize.y && pz < p.gridSize.z)
    {
        ...
        else if (p.loss == LOSS_N2N)
            vloss = vec3f(fwdN2N(img.x, target.x), fwdN2N(img.y, target.y), fwdN2N(img.z, target.z));
...
{ 
    ...
    {
        bwdN2N(img.x, target.x, d_img.x, d_target.x, d_vloss.x);
        bwdN2N(img.y, target.y, d_img.y, d_target.y, d_vloss.y);
        bwdN2N(img.z, target.z, d_img.z, d_target.z, d_vloss.z);
    }
    else if (p.loss == LOSS_SMAPE)
    {
